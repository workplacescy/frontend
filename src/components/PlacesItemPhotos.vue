...
<template>
  <swiper :modules="modules" :navigation="{disabledClass: 'd-none', prevEl: '.navigation-prev', nextEl: '.navigation-next'}" :preload-images="false" :space-between="8" grab-cursor slides-per-view="2.5" @mouseenter="navigationVisible = true" @mouseleave="navigationVisible = false">
    <swiper-slide v-for="(photo, index) in props.photos" v-once :key="photo.thumb.src">
      <v-img :alt="photo.thumb.alt" :src="photo.thumb.src" :title="photo.thumb.caption" :width="photo.thumb.width" aspect-ratio="1" cover="" lazy-src="/src/assets/images/image-area.svg" @click="$emit('clickPhoto', index)"/>
    </swiper-slide>
    <v-fade-transition>
